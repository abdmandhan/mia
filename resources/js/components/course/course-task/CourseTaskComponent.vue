...
export default {
  ...
  methods: {
    initialize() {
      this.loading = true;
      ApiService.get(`course-task/${this.$route.params.course_id}`).then(
        (result) => {
    ...
    deleteItemConfirm() {
      ApiService.delete(`course-task/${this.currentItem.id}`).then((result) => {
