...
export default {
  data: () => ({
    ...
    headers: [
      { text: "ID", value: "id" },
      { text: "Course Type", value: "course_type.name" },
      { text: "Name", value: "name" },
      { text: "Premium", value: "is_premium" },
  ...
  methods: {
    initialize() {
      this.loading = true;
      ApiService.get("course").then((result) => {
    ...
    createItem() {
      this.$router.push({ name: "course-management.create" });
    },
    ...
    deleteItemConfirm() {
      ApiService.delete(`course/${this.currentItem.id}`).then((result) => {
