...
export default {
  ...
  methods: {
    initialize() {
      this.loading = true;
      ApiService.get(
        `course-task-question/${this.$route.params.course_task_id}`
      ).then((result) => {
    ...
    editItem(item) {
      this.$router.push({
        ...
        params: { course_task_question_id: item.id },
      });
    },
    ...
    deleteItemConfirm() {
      ApiService.delete(`course-task-question/${this.currentItem.id}`).then(
        (result) => {
