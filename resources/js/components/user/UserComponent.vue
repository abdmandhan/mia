...
export default {
  data: () => ({
    ...
    headers: [
      { text: "ID", value: "id" },
      { text: "User Type", value: "user_type.name" },
  ...
  methods: {
    initialize() {
      this.loading = true;
      ApiService.get("user").then((result) => {
    ...
    createItem() {
      this.$router.push({ name: "user-management.create" });
    },
    ...
    deleteItemConfirm() {
      ApiService.delete(`user/${this.currentItem.id}`).then((result) => {
